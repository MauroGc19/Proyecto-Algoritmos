        ...
        for linea in self.tokens:
            ...
            for i, token in enumerate(linea):
                if token in operadores_booleanos:
                    ...
                    if token in {"and", "or"}:
                        if i == 0 or i == len(linea) - 1:
                            self.errores.append(
                                f'Error: Operador booleano "{token}" mal posicionado en la línea: '
                                + " ".join(linea)
                            )
                        """
                ...
                if token in operadores_relacionales:
                    if i == 0 or i == len(linea) - 1:
                        self.errores.append(
                            f'Error: Operador relacional "{token}" mal posicionado en la línea: '
                            + " ".join(linea)
                        )
                    else:
                        op_izq = linea[i - 1]
                ...
                if token in operadores_matematicos:
                    if token in {"┌", "└"}:
                        ...
                        if i == len(linea) - 1 or linea[i + 2] not in {"┐", "┘"}:
                            self.errores.append(
                                f'Error: Operador matemático "{token}" sin cierre correcto en la línea: '
                                + " ".join(linea)
                            )
                    elif token in {"┐", "┘"}:
                        if i == 0 or linea[i - 2] not in {"┌", "└"}:
                            self.errores.append(
                                f'Error: Operador matemático "{token}" sin apertura correcta en la línea: '
                                + " ".join(linea)
                            )
                    else:
                        if i == 0 or i == len(linea) - 1:
                            self.errores.append(
                                f'Error: Operador matemático "{token}" mal posicionado en la línea: '
                                + " ".join(linea)
                            )
                ...
                try:
                    ...
                    for atributo in atributos:
                        if not atributo.isidentifier():
                            self.errores.append(
                                f'Error: Nombre de atributo inválido "{atributo}" en clase {nombre_clase}'
                            )
                    self.clases_definidas[nombre_clase] = atributos
                except Exception:
                    self.errores.append(
                        "Error: Definición de clase mal formada en la línea: "
                        + " ".join(linea)
                    )
            ...
            if (
                len(linea) >= 6
                and linea[0].lower() == "for"
                and linea[3].lower() == "to"
                and linea[5].lower() == "do"
            ):
                en_for = True
                if not linea[1].isidentifier():
                    self.errores.append(
                        f"Error: Variable contadora inválida en FOR: {linea[1]}"
                    )
            ...
            if (
                len(linea) >= 5
                and linea[0].lower() == "while"
                and linea[2].lower() == "do"
            ):
                en_while = True
                # linea[1] debería ser la condición entre paréntesis
                if not (linea[1].startswith("(") and linea[1].endswith(")")):
                    self.errores.append(
                        "Error: Condición de WHILE debe estar entre paréntesis"
                    )
            ...
            if en_repeat and len(linea) >= 2 and linea[0].lower() == "until":
                ...
                if not (linea[1].startswith("(") and linea[1].endswith(")")):
                    self.errores.append(
                        "Error: Condición de UNTIL debe estar entre paréntesis"
                    )
                en_repeat = False
            ...
            if (
                len(linea) >= 3
                and linea[0].lower() == "if"
                and linea[2].lower() == "then"
            ):
                en_if = True
                if not (linea[1].startswith("(") and linea[1].endswith(")")):
                    self.errores.append(
                        "Error: Condición de IF debe estar entre paréntesis"
                    )
            ...
            if len(linea) >= 2 and linea[0] in self.clases_definidas:
                ...
                if not nombre_objeto.isidentifier():
                    self.errores.append(
                        f'Error: Nombre de objeto inválido "{nombre_objeto}"'
                    )
                else:
            ...
            if "🡨" in linea:
                if linea.count("🡨") > 1:
                    self.errores.append(
                        "Error: Asignación múltiple no permitida en la línea: "
                        + " ".join(linea)
                    )
            ...
            if any(palabra.lower() == "procedure" for palabra in linea):
                ...
                self.arreglos_declarados = set()
                self.despues_de_begin = False
            if self.en_procedimiento and any(
                palabra.lower() == "begin" for palabra in linea
            ):
            ...
            for palabra in linea:
                if "[" in palabra and "]" in palabra:
                    ...
                    if nombre not in self.arreglos_declarados:
                        self.errores.append(
                            f'Error: El arreglo "{nombre}" no ha sido declarado en la línea: '
                            + " ".join(linea)
                        )
                if ".." in palabra:
                    ...
                    if not (("[" in palabra and "]" in palabra)):
                        self.errores.append(
                            f"Error: Notación de rango mal usada en la línea: "
                            + " ".join(linea)
                        )
                ...
                if "." in palabra:
                    ...
                    if len(partes) == 2:
                        ...
                        if objeto not in self.objetos_declarados:
                            self.errores.append(
                                f'Error: El objeto "{objeto}" no ha sido declarado en la línea: '
                                + " ".join(linea)
                            )
                        else:
                            clase = self.objetos_declarados[objeto]
                            if (
                                clase in self.clases_definidas
                                and campo not in self.clases_definidas[clase]
                            ):
                                self.errores.append(
                                    f'Error: El campo "{campo}" no existe en la clase {clase} en la línea: '
                                    + " ".join(linea)
                                )
                    else:
                        self.errores.append(
                            f"Error: Acceso a campo mal formado en la línea: "
                            + " ".join(linea)
                        )
